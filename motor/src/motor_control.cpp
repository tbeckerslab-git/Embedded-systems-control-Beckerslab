  ...
  while (true) {
    ...
    try {
      ...
      if (desiredPosition < 0 || desiredPosition > 4095) {
        std::cout << "Invalid position. Please enter a position within the "
                     "range 0-4095."
                  << std::endl;
        continue;
      }
      ...
      std::cout << "Invalid input. Please enter a numeric position within the "
                   "range 0-4095."
                << std::endl;
    }
  }
...
int initializeDynamixel() {
  ...
  int dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL_ID,
...
void setGoalPosition(int position) {
  int dxl_comm_result = packetHandler->write4ByteTxRx(
      portHandler, DXL_ID, ADDR_MX_GOAL_POSITION, position);
  if (dxl_comm_result != COMM_SUCCESS) {
    printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
  } else {
    uint8_t dxl_error = 0;
